...
# ==========================================
# 1. DESIGN PALETTE & CUSTOM STYLING ONLY
# ==========================================
# Standard HTML markdown is used here instead of page config to completely bypass character errors.
st.markdown("""
    <style>
    ...
    /* Ensure Sidebar text matches the theme design */
...
def load_data():
    username = "aarongaming2712-star"
    repo = "gdp-dashboard-1"
    filename = "Train.csv"
    ...
    try:
        ...
        st.sidebar.warning(f"⚠️ Sandbox Mode: Running on simulated data.")
        np.random.seed(42)
        n = 1000
        return pd.DataFrame({
            'ID': np.arange(1, n + 1),
...
with st.sidebar:
    ...
    st.markdown("---")
    st.info("💡 **Accuracy Quality Framework Engine Active**")
...
# Initialize session state tracking rules
...
# Apply real-time simulated discrepancies to operational copy
...
def show_registry_page():
    st.title("📋 Registry: Accuracy Dimension Breakdown")
    st.markdown("Detailed configuration of data quality rules and enforcement tracking metrics across the data scheme.")
    
    with st.status("Analyzing compliance constraints...", expanded=False) as status_box:
        time.sleep(0.1)
        status_box.update(label="Registry Compliance Profiles Loaded Successfully!", state="complete", expanded=False)
    ...
    st.caption(f"Displaying {len(audit_display_df):,} records based on filter metrics.")
...
def show_raw_dataset_page():
    ...
    selected_column = st.selectbox("Choose a specific column to isolate and analyze:", options=column_options, index=0)
